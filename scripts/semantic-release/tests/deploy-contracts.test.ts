...
// Mock csv-parse library with more realistic behavior
jest.mock('csv-parse/sync', () => {
  return {
    parse: jest.fn().mockImplementation((input, options) => {
      // Return empty array for empty input
      if (!input || !input.trim()) return []
      
      // Convert input to array of objects with the specified column names
      const rows = input.split('\n')
        .filter(line => line.trim())
        .filter(line => !line.startsWith(options.comment || '#'))
        .map(line => {
          const values = line.split(options.delimiter || ',').map(val => val.trim())
          
          // If columns option is provided, create an object with column names
          if (options.columns) {
            ...
            options.columns.forEach((col: string, index: number) => {
              ...
              if (options.cast && typeof options.cast === 'function') {
                value = options.cast(value, { column: col })
              }
...
describe('Deploy Contracts Module', () => {
  ...
  describe('parseDeploymentResults', () => {
    ...
    it('should parse valid CSV deployment records', () => {
      fs.readFileSync.mockReturnValueOnce(`
        1,0xAddress1,contracts/Contract1.sol:Contract1
        # This is a comment
        10,0xAddress2,contracts/Contract2.sol:Contract2
        42,0xAddress3,contracts/Contract3.sol:Contract3
      `)
      
      const result = parseDeploymentResults('/path/to/results.csv', mockLogger)
      
      expect(result).toHaveLength(3)
      expect(result).toEqual([
        { address: '0xAddress1', name: 'Contract1', chainId: 1 },
        { address: '0xAddress2', name: 'Contract2', chainId: 10 },
        { address: '0xAddress3', name: 'Contract3', chainId: 42 }
      ])
    })
    ...
    it('should handle empty files', () => {
      ...
      const result = parseDeploymentResults('/path/to/empty.csv', mockLogger)
      
      expect(result).toEqual([])
      expect(mockLogger.log).toHaveBeenCalledWith('Deployment results file is empty: /path/to/empty.csv')
    })
    
    it('should handle missing files', () => {
      fs.existsSync.mockReturnValueOnce(false)
      
      const result = parseDeploymentResults('/path/to/nonexistent.csv', mockLogger)
      
      expect(result).toEqual([])
      expect(mockLogger.log).toHaveBeenCalledWith('Deployment results file not found: /path/to/nonexistent.csv')
    })
    ...
    it('should filter out invalid records', () => {
      fs.readFileSync.mockReturnValueOnce(`
        1,0xAddress1,contracts/Contract1.sol:Contract1
        invalid-line
        10,,contracts/Contract2.sol:Contract2
        42,0xAddress3,
      `)
      ...
      expect(mockLogger.log).toHaveBeenCalledTimes(3) // One call for each invalid record
    })
    
    it('should handle parsing errors gracefully', () => {
      // Mock readFileSync to return content
      fs.readFileSync.mockReturnValueOnce('valid,csv,content')
      
      // Mock csv-parse to throw an error
      const parseMock = require('csv-parse/sync').parse
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        'Error parsing deployment results from /path/to/results.csv: CSV parsing error'
      )
    })
    
    it('should work without a logger', () => {
      fs.readFileSync.mockReturnValueOnce('1,0xAddress1,contracts/Contract1.sol:Contract1')
      
      // Mock console.error to verify it gets called when no logger provided
      const consoleErrorSpy = jest.spyOn(console, 'error').mockImplementation()
      
      // Mock csv-parse to throw an error for the second test case
      const parseMock = require('csv-parse/sync').parse
      parseMock.mockImplementationOnce(() => [{ chainId: 1, address: '0xAddress1', contractPath: 'contracts/Contract1.sol:Contract1' }])
      
      const result = parseDeploymentResults('/path/to/results.csv')
      ...
      // Reset the parseMock for the next test
      parseMock.mockImplementationOnce(() => {
        throw new Error('CSV parsing error')
      })
      
      // Test error handling without logger
      const errorResult = parseDeploymentResults('/path/to/results.csv')
      
      expect(errorResult).toEqual([])
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        'Error parsing deployment results: CSV parsing error'
      )
