...
/**
 * @title MockGateway
 * @notice Mock Gateway contract for testing depositFor functionality
 */
...
contract DepositIntegration_GatewayDepositTest is Test {
     ...
     */
    function test_integration_fullGatewayDepositFlow() public {
    ...
    function test_integration_createAndFulfillIntent() public {
        ...
        // 2. Fulfill the intent (spending solver's route tokens, which go to Executor, then Gateway is called)
        ...
        //    (They stay with Executor since MockGateway doesn't transfer them)
        ...
        // 2. Verify Gateway depositFor was called correctly by checking events
        ...
        for (uint256 i = 0; i < fulfillLogs.length; i++) {
            if (fulfillLogs[i].topics[0] == keccak256("DepositFor(address,address,uint256)")) {
                (address token_, address recipient, uint256 amount_) = abi.decode(
                    fulfillLogs[i].data,
        ...
        assertTrue(foundDepositFor, "DepositFor event should be emitted");
     ...
     */
    function test_integration_multipleGatewayDeposits() public {
        address deployed = factory.deploy(USER_DESTINATION, DEPOSITOR);
        DepositAddress_GatewayDeposit depositAddress = DepositAddress_GatewayDeposit(deployed);
    ...
    /**
     * @notice Test Gateway call is constructed correctly by checking emitted events
     */
    function test_integration_gatewayCallConstruction() public {
        address deployed = factory.deploy(USER_DESTINATION, DEPOSITOR);
        DepositAddress_GatewayDeposit depositAddress = DepositAddress_GatewayDeposit(deployed);
        ...
        assertTrue(foundIntentPublished, "IntentPublished event should be emitted with Gateway call");
    }

    /**
     * @notice Test gas estimation for Gateway flow
     */
