...
# deployMulticall3.sh
#
# This script handles the deployment of the canonical Multicall3 contract.
# Multicall3 enables batching multiple calls into a single transaction,
# returning the results in a single response, which is crucial for efficient
# blockchain interactions.
#
# Features:
# - Checks if the Multicall3 contract already exists at the canonical address
# - Deploys the contract using a specific private key if not present
...
# Address that will deploy the Multicall3 contract
MULTICALL3_DEPLOYER="0xBb6e024b9cFFACB947A71991E386681B1Cd1477D"

# Standard Multicall3 address
MULTICALL3_ADDRESS="0xcA11bde05977b3631167028862bE2a173976CA11"

# Signed transaction to deploy the Multicall3 - this is the exact same for all chains
...
echo "$CHAIN_JSON" | jq -c 'to_entries[]' | while IFS= read -r entry; do
  ...
  if [ "$code" == "0x" ]; then
    echo "🔄 Preparing to publish Multicall3 on Chain ID: $CHAIN_ID"

    # Check if we need to fund the deployer account
    balance=$(cast balance $MULTICALL3_DEPLOYER --rpc-url "$RPC_URL")
    # Convert to eth for comparison
    balance_eth=$(cast --from-wei "$balance")
    # 0.0247 ETH + .00001 ETH buffer
    MULTICALL3_COST=0.02471
    # Check if the balance is less than MULTICALL3_COST
    if (($(echo "$balance_eth < $MULTICALL3_COST" | bc -l))); then
      # We need to fund the account with exactly 0.0247 ETH
      echo "💰 Funding deployer account with exactly $MULTICALL3_COST ETH..."

      # Calculate how much to send (0.0247 - current balance)
      to_send=$(echo "$MULTICALL3_COST - $balance_eth" | bc -l)
      ...
      new_balance=$(cast balance $MULTICALL3_DEPLOYER --rpc-url "$RPC_URL")
    ...
    echo "🚀 Publishing Multicall3 transaction..."
    tx=$(cast publish $SIGNED_TRANSACTION --rpc-url "$RPC_URL")
    echo "Publishing Multicall3 transaction: $tx"
    tx_hash=$(echo "$tx" | grep "transactionHash" | awk '{print $2}')
    if [ -n "$tx_hash" ]; then
      ...
      code=$(cast code $MULTICALL3_ADDRESS --rpc-url "$RPC_URL")
      if [ "$code" != "0x" ]; then
        echo "✅ Multicall3 successfully deployed on Chain ID: $CHAIN_ID"
      else
    ...
    echo "✅ Multicall3 already deployed on Chain ID: $CHAIN_ID"
  fi

done
echo "Multicall3 deployment check completed."
