...
contract LayerZeroProverDeploy is Script {
    ...
    function run() external {
        ...
        deployLayerZeroProver(deployer);
        
        vm.stopBroadcast();
    }

    function deployLayerZeroProver(address deployer) public returns (address) {
        ...
        // Read OP_PROVERS from environment variable (optional)
        bytes32[] memory provers;
        try vm.envBytes32("OP_PROVERS", ",") returns (bytes32[] memory envProvers) {
            provers = envProvers;
        } catch {
            provers = new bytes32[](0);
        }

        // Get salt from environment or use default
        bytes32 salt = vm.envBytes32("SALT");
        
        // Calculate contract salt
        bytes32 contractSalt = getContractSalt(salt, "LAYERZERO_PROVER");
        
        // Prepare constructor arguments
        bytes memory constructorArgs = abi.encode(
            endpoint,
            deployer, // Use deployer as delegate
            portal,
            provers,
            defaultGasLimit
        );
        ...
        // Predict the address
        address predictedAddress = predictCreate3Address(bytecode, deployer, contractSalt);
        console.log("=== LAYERZERO PROVER DEPLOYMENT ===");
        console.log("Predicted Address:", predictedAddress);
        console.log("Endpoint:", endpoint);
        console.log("Portal:", portal);
        console.log("Default Gas Limit:", defaultGasLimit);
        console.log("Provers Count:", provers.length);
        console.log("Salt:", vm.toString(contractSalt));
        console.log("================================");

        // Check if already deployed
        if (isDeployed(predictedAddress)) {
            console.log("LayerZeroProver already deployed at:", predictedAddress);
            return predictedAddress;
        }

        // Deploy using CREATE3
        (address deployedAddress, bool deployed) = deployWithCreate3(bytecode, deployer, contractSalt);
        
        if (deployed) {
            console.log("LayerZeroProver deployed successfully at:", deployedAddress);
        } else {
            console.log("LayerZeroProver deployment failed");
        }
    ...
    function predictAddress() external view {
        ...
        console.log("=== LAYERZERO PROVER ADDRESS PREDICTION ===");
        console.log("Predicted Address:", predictedAddress);
        console.log("Endpoint:", endpoint);
        console.log("Portal:", portal);
        console.log("Default Gas Limit:", defaultGasLimit);
    ...
    function deployWithCreate3(
        ...
        deployedContract = create3Deployer.deployedAddress(bytecode, sender, salt);
    ...
    function deployCreate3Deployer() internal {
        if (!isDeployed(address(create3Deployer))) {
            ...
            console.log("Deployed Create3Deployer:", address(create3Deployer));
        } else {
            console.log("Create3Deployer already deployed at address:", address(create3Deployer));
        }
    }
