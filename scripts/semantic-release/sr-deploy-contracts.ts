...
import { deployTronAndEVMContracts } from './sr-deploy-tron'
...
export async function deployRoutesContracts(
  ...
  try {
    ...
    // Check if TRON deployment is enabled
    const enableTronDeployment = process.env.ENABLE_TRON_DEPLOYMENT === 'true'

    let contracts: Contract[] = []
    let success = false

    if (enableTronDeployment) {
      // Deploy using integrated Tron + EVM workflow
      logger.log('🔗 Deploying contracts with Tron + EVM integration...')
      await deployTronAndEVMContracts(context, rootSalt, preprodRootSalt)

      // Parse all deployment results
      contracts = await parseAllDeploymentResults(logger, cwd)
      success = contracts.length > 0
    } else {
      // Legacy deployment workflow
      logger.log('⚡ Deploying contracts with legacy EVM-only workflow...')
...
/**
 * Parses all deployment results from both Tron and EVM deployments when using integrated workflow.
 * Combines results from multiple CSV files into a single contract array.
 *
 * @param logger - Logger instance for output messages
 * @param cwd - Current working directory
 * @returns Array of Contract objects from all deployments
 */
async function parseAllDeploymentResults(
  logger: Logger,
  cwd: string,
): Promise<Contract[]> {
  let allContracts: Contract[] = []

  // Parse Tron deployment results
  const tronResultsPath = path.join(cwd, 'out', 'tron-deploy.csv')
  if (fs.existsSync(tronResultsPath)) {
    const tronContracts = parseDeploymentResults(
      tronResultsPath,
      'tron',
      logger,
    )
    allContracts = [...allContracts, ...tronContracts]
  }

  // Parse EVM deployment results
  const evmProdResultsPath = path.join(cwd, 'out', 'evm-deploy-production.csv')
  if (fs.existsSync(evmProdResultsPath)) {
    const evmProdContracts = parseDeploymentResults(
      evmProdResultsPath,
      'default',
      logger,
    )
    allContracts = [...allContracts, ...evmProdContracts]
  }

  const evmPreResultsPath = path.join(
    cwd,
    'out',
    'evm-deploy-pre-production.csv',
  )
  if (fs.existsSync(evmPreResultsPath)) {
    const evmPreContracts = parseDeploymentResults(
      evmPreResultsPath,
      'pre',
      logger,
    )
    allContracts = [...allContracts, ...evmPreContracts]
  }

  logger.log(
    `Parsed ${allContracts.length} total contracts from all deployments`,
  )
  return allContracts
}
 ...
 * @param deployEnv - Environment name (e.g., 'default', 'pre', 'tron') for categorizing results
