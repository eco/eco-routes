...
contract UniversalSourceTest is BaseTest {
    ...
    function _setupUniversalTestData() internal {
        ...
        universalRouteTokens.push(UniversalTokenAmount({
            token: address(tokenA).toBytes32(),
            amount: MINT_AMOUNT
        }));
        
        // Setup universal calls
        universalCalls.push(UniversalCall({
            target: address(tokenA).toBytes32(),
            data: abi.encodeWithSignature("transfer(address,uint256)", creator, MINT_AMOUNT),
            value: 0
        }));
        
        // Setup universal reward tokens
        universalRewardTokens.push(UniversalTokenAmount({
            token: address(tokenA).toBytes32(),
            amount: MINT_AMOUNT
        }));
        universalRewardTokens.push(UniversalTokenAmount({
        ...
        UniversalTokenAmount[] memory universalRouteTokensMemory = new UniversalTokenAmount[](universalRouteTokens.length);
        for (uint256 i = 0; i < universalRouteTokens.length; i++) {
            universalRouteTokensMemory[i] = universalRouteTokens[i];
        }
        
        UniversalCall[] memory universalCallsMemory = new UniversalCall[](universalCalls.length);
        for (uint256 i = 0; i < universalCalls.length; i++) {
            universalCallsMemory[i] = universalCalls[i];
        }
        
        UniversalTokenAmount[] memory universalRewardTokensMemory = new UniversalTokenAmount[](universalRewardTokens.length);
    ...
    function _convertToEVMIntent(UniversalIntent memory _universalIntent) internal pure returns (EVMIntent memory) {
        // Convert route tokens
        EVMTokenAmount[] memory evmRouteTokens = new EVMTokenAmount[](_universalIntent.route.tokens.length);
        ...
        // Convert calls
        EVMCall[] memory evmCalls = new EVMCall[](_universalIntent.route.calls.length);
        ...
        EVMTokenAmount[] memory evmRewardTokens = new EVMTokenAmount[](_universalIntent.reward.tokens.length);
    ...
    function testUniversalIntentHashing() public {
        (bytes32 universalHash,,) = universalSource.getIntentHash(universalIntent);
    ...
    function testUniversalIntentVaultAddress() public {
        address universalVault = universalSource.intentVaultAddress(universalIntent);
    ...
    function testPublishUniversalIntent() public {
        ...
        address intentVault = universalSource.intentVaultAddress(universalIntent);
        
        vm.startPrank(creator);
        tokenA.approve(intentVault, MINT_AMOUNT);
        tokenB.approve(intentVault, MINT_AMOUNT * 2);
        
        universalSource.fundFor(routeHash, universalIntent.reward, creator, address(0), false);
        vm.stopPrank();
    ...
    function testFundUniversalIntentWithNativeReward() public {
        ...
        vm.prank(creator);
        universalSource.publishAndFund{value: REWARD_NATIVE_ETH}(universalIntent, false);
        
        assertTrue(universalSource.isIntentFunded(universalIntent));
        
        address vaultAddress = universalSource.intentVaultAddress(universalIntent);
    ...
    function testInsufficientNativeRewardUniversal() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IBaseSource.InsufficientNativeReward.selector, keccak256(abi.encodePacked(keccak256(abi.encode(universalIntent.route)), keccak256(abi.encode(universalIntent.reward))))));
        vm.prank(creator);
        universalSource.publishAndFund{value: 0.5 ether}(universalIntent, false);
    }
    ...
    function testWrongSourceChainUniversal() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IBaseSource.WrongSourceChain.selector, keccak256(abi.encodePacked(keccak256(abi.encode(universalIntent.route)), keccak256(abi.encode(universalIntent.reward))))));
        vm.prank(creator);
        universalSource.publishAndFund(universalIntent, false);
    }
}