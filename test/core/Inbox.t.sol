...
contract InboxTest is BaseTest {
    ...
    function testInboxCanReceiveIntents() public {
        ...
        vm.prank(solver);
        inbox.fulfill(intent.route, rewardHash, bytes32(uint256(uint160(recipient))), intentHash, address(0));
    }
    ...
    function testInboxFulfillWithNonAddressClaimant() public {
        ...
        vm.prank(solver);
        inbox.fulfill(intent.route, rewardHash, nonAddressClaimant, intentHash, address(0));
    ...
    function testFulfillAndProveIntegration() public {
        ...
        vm.prank(solver);
        inbox.fulfillAndProve(intent.route, rewardHash, claimantBytes, intentHash, address(prover), "");
    ...
    function testInitiateProvingWithMultipleIntents() public {
        ...
        vm.prank(solver);
        inbox.initiateProving{value: 1 ether}(block.chainid, intentHashes, address(prover), "test_data");
    }
    ...
    function testFulfillRejectsAlreadyFulfilled() public {
        ...
        vm.prank(solver);
        inbox.fulfill(intent.route, rewardHash, claimantBytes, intentHash, address(0));
    ...
    function testFulfillEmitsCorrectEvent() public {
        ...
        _expectEmit();
        emit IInbox.Fulfillment(intentHash, intent.route.source, address(0), claimantBytes);
    ...
    function testFulfillWithZeroClaimant() public {
        ...
        vm.expectRevert();
        vm.prank(solver);
        inbox.fulfill(intent.route, rewardHash, bytes32(0), intentHash, address(0));
    }
    ...
    function _createIntent() internal view returns (Intent memory) {
        ...
        calls[0] = Call({
            ...
            data: abi.encodeWithSignature("transfer(address,uint256)", recipient, MINT_AMOUNT),
            value: 0
        });
    ...
    function _createIntentWithMultipleTokens() internal view returns (Intent memory) {
        TokenAmount[] memory tokens = new TokenAmount[](2);
        tokens[0] = TokenAmount({token: address(tokenA), amount: MINT_AMOUNT});
        tokens[1] = TokenAmount({token: address(tokenB), amount: MINT_AMOUNT * 2});
        ...
        calls[1] = Call({
            ...
            data: abi.encodeWithSignature("transfer(address,uint256)", recipient, MINT_AMOUNT * 2),
            value: 0
        });
