 ...
 * @notice Test prover whose provenIntents() SUCCEEDS and returns exactly 96
 *         bytes (the correct ProofData shape) but with every bit set,
 *         including the upper 96 bits of the address word, the upper 192
 *         bits of the uint64 word and every bit of the outcome word
 ...
 *      exact payload directly to ProofData would revert in the
 ...
 *      prove the wide decode plus range check avoids that.
 */
