...
> cross-chain fact and require the claimant to supply the preimage at settle. Have the **Account call the
...
> contract) and the single-sided Account + Executor — the transport/policy relay split, the unified
> dual-chain account, and the runtime are later stages. Stacked on PR1 (`v3/01-policy-storage`). Net-new,
...
struct Reward { uint64 deadline; address keeper; address prover; RewardToken[] tokens; }
```
...
**`keeper`** — the owner of the DESTINATION-side account:

```solidity
struct Route {
    bytes32 salt; uint64 deadline; address portal;
    address keeper;                  // owner of the DESTINATION-side account (leftover / executeAsOwner authority)
    Call[] calls;
    TokenAmount[] minTokens;          // minimum inputs the solver must PROVIDE (may provide more)
}
```

- `route.keeper` is the **destination-side account owner** — the authority that may retrieve the leftover
  and (once account execution lands in PR3) `executeAsOwner`. It lives in the **Route** because the
  destination only sees the route plus the opaque `rewardHash` and **cannot read `Reward.keeper`**, so
  without it the destination has no way to authenticate who owns the account. It is the **same logical
...
- `route.minTokens[j] = (inToken_j, minAmount_j)`: the solver must **provide at least** `minAmount_j` of
  ...
  not sent to a protocol recipient** — it is moved to the **intent's Account** so the leftover **stays with
  the intent**, where `route.keeper` can retrieve it later. Native folds in as a `minTokens` leg with
...
- **Per-leg reward** = `RewardMath.reward(fulfilled_j, rate_j, flat_j) = mulDiv(fulfilled_j, rate_j, WAD)
  ...
  Always **capped at the account balance** for that token (`RewardMath.capped`) — never release more than
...
- Reward legs MUST be **unique by token** and `<= MAX_REWARD_TOKENS` (`IntentLib.requireUniqueRewardTokens`,
  ...
  compatibility), so the paired-vs-`minTokens` ORDER is a keeper-side canonical form; only the route-free
...
- **There is NO recipient and NO output balance-delta measurement.** Delivery is the calls' job (any
  ...
  `Executor.sweepTo(route.minTokens, intentAccount)` moves any input the calls did not consume to the
  **intent's Account** — leftover stays **with the intent** (the keeper retrieves it later) rather than
  being stranded in the shared executor or sent to a protocol-level recipient. The intent Account address is
  deterministic (CREATE2 keyed on the intent hash) and identical across chains, so the same per-intent
  account the keeper controls on the source chain is addressable from the destination Inbox; the composition
  root (Portal / PortalTron) wires `_predictAccount` to `IntentSource._getAccount`. The Executor holds **only**
  solver input (never reward escrow — escrow lives in the Account), so moving its remainder to the Account can
  never misdirect escrow. `fulfillmentHash` is computed over `fulfilled[] = providedAmounts` and recorded
  into the named prover **after** execution + the account move; a re-entrant re-fulfill of the same intent
...
## 3. Settlement — preimage verify + Account-calls-Policy(view)
...
4. `Account.withdraw(reward, claimant, fulfilled)` — the **Account** calls
   ...
   residual of each leg token to `reward.keeper`**.
...
at settle only out of account balance in excess of the flats, always capped at balance. A fixed same-asset
...
conventions are preserved. `refundTo(refundee)` is **restored**; `recoverToken` + `Account.recover` +
...
keeper's funds) is preserved by the **deadline** instead. Consequence: a proven-but-unsettled
...
intent Account runs in the separately-deployed `Executor`, which does not count against the Portal). With
...
optimization at no size cost. (Net: `recipient` removed and `keeper` — the destination account owner —
added back in its place; headroom is 568 bytes, still comfortably under the limit.)

## 5. What is deferred (NOT in PR2)
- **runtime / delegatecall execution + unified dual-chain account** → PR3 (Route keeps `salt`, `keeper`,
  `calls`, `minTokens`; `route.keeper` becomes the executeAsOwner authority there; still the single-sided
  Account + Executor here).
- **`source` in the intent hash / chain-parameterized account** → PR3.
