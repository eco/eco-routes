...
import { keccak256, BytesLike, ZeroAddress, getCreate2Address, solidityPacked, AbiCoder } from 'ethers'
...
describe('Universal Intent Source Test', (): void => {
  ...
  function hashUniversalIntent(intent: UniversalIntent) {
    ...
    const routeHash = keccak256(AbiCoder.defaultAbiCoder().encode(
    ...
    const rewardHash = keccak256(AbiCoder.defaultAbiCoder().encode(
        ...
        solidityPacked(
          ...
          [
            ...
            abiCoder.encode(
              ...
              [intentHash, [intent.reward.creator, intent.reward.prover, intent.reward.deadline, intent.reward.nativeValue, rewardTokens]],
            ),
          ],
        ),
      ),
  ...
  async function deployFixture() {
    ...
    const addressConverterTestFactory = await ethers.getContractFactory('AddressConverterTest')
    ...
    const badToken = await badTokenFactory.deploy('BadToken', 'BAD', await creator.getAddress())
  ...
  async function mintAndApprove() {
    await tokenA.connect(creator).mint(await creator.getAddress(), Number(mintAmount))
    await tokenB.connect(creator).mint(await creator.getAddress(), Number(mintAmount * 2n))

    await tokenA.connect(creator).approve(await intentSource.getAddress(), mintAmount)
    await tokenB.connect(creator).approve(await intentSource.getAddress(), mintAmount * 2n)
  }
    ...
    calls = [
      {
        ...
        data: await encodeTransfer(await creator.getAddress(), Number(mintAmount)),
        value: 0n
      }
    ]
  ...
  describe('Address conversion', function() {
    ...
    it('should correctly identify valid Ethereum addresses in bytes32', async function() {
      ...
      const validBytes32 = await addressConverter.toBytes32(await creator.getAddress())
      
      // Invalid Ethereum address (has data in top 12 bytes)
      const invalidBytes32 = ethers.hexlify(ethers.randomBytes(32))
      
      const isValid = await addressConverter.isValidEthereumAddress(validBytes32)
      const isInvalid = await addressConverter.isValidEthereumAddress(invalidBytes32)
  ...
  describe('Intent publishing', function() {
    it('should successfully publish a standard intent', async function() {
      ...
      const events = await intentSource.queryFilter(filter, receipt?.blockNumber, receipt?.blockNumber)
    ...
    it('should successfully publish a universal intent', async function() {
      const tx = await universalIntentSource.connect(creator).publish(universalIntent)
      ...
      // Verify intent is not funded yet
      expect(await universalIntentSource.isIntentFunded(universalIntent)).to.be.false
    })
  })
  ...
  describe('Intent funding', function() {
    it('should properly fund an EVM intent with ERC20 tokens', async function() {
      ...
      expect(await tokenB.balanceOf(vaultAddress)).to.equal(Number(mintAmount * 2n))
    })
    
    it('should properly fund a universal intent with ERC20 tokens', async function() {
      // Get intent hash to verify events
      const [intentHash] = await universalIntentSource.getIntentHash(universalIntent)
      ...
      // Verify funding status
      expect(await universalIntentSource.isIntentFunded(universalIntent)).to.be.true
      
      // Check vault balance for each token
      const vaultAddress = await universalIntentSource.intentVaultAddress(universalIntent)
      expect(await tokenA.balanceOf(vaultAddress)).to.equal(Number(mintAmount))
      expect(await tokenB.balanceOf(vaultAddress)).to.equal(Number(mintAmount * 2n))
    })
    ...
    it('should handle funding with overpayment of native token', async function() {
      ...
      const vaultAddress = await intentSource.intentVaultAddress(evmIntent)
      expect(await ethers.provider.getBalance(vaultAddress)).to.eq(evmIntent.reward.nativeValue)
      ...
      expect(initialBalance - finalBalance).to.be.lessThan(evmIntent.reward.nativeValue * 2n)
    })
    ...
    it('should allow partial funding when allowPartial is true', async function() {
      ...
      await tokenA.connect(creator).approve(await intentSource.getAddress(), mintAmount / 2n)
      await tokenB.connect(creator).approve(await intentSource.getAddress(), mintAmount) 
      ...
      const vaultAddress = await intentSource.intentVaultAddress(evmIntent)
      expect(await tokenA.balanceOf(vaultAddress)).to.equal(Number(mintAmount / 2n))
  ...
  describe('Reward claiming', function() {
    ...
    it('should allow claiming rewards for a proven intent', async function() {
      ...
      // Initial balances
      const initialEthBalance = await ethers.provider.getBalance(await claimant.getAddress())
      const initialTokenABalance = await tokenA.balanceOf(await claimant.getAddress())
      const initialTokenBBalance = await tokenB.balanceOf(await claimant.getAddress())
      
      // Withdraw rewards
      await intentSource.connect(otherPerson).withdrawRewards(
        routeHash,
        evmIntent.reward
      )
      
      // Final balances
      const finalEthBalance = await ethers.provider.getBalance(await claimant.getAddress())
      const finalTokenABalance = await tokenA.balanceOf(await claimant.getAddress())
      const finalTokenBBalance = await tokenB.balanceOf(await claimant.getAddress())
      
      // Verify claimant received rewards
      expect(finalEthBalance).to.equal(initialEthBalance + evmIntent.reward.nativeValue)
      expect(finalTokenABalance).to.equal(initialTokenABalance + mintAmount)
      expect(finalTokenBBalance).to.equal(initialTokenBBalance + (mintAmount * 2n))
    })
    
    it('should update reward status after withdrawal', async function() {
      // Get intent hash and route hash
      const [intentHash, routeHash] = await intentSource.getIntentHash(evmIntent)
    ...
    it('should emit Withdrawal event when rewards are claimed', async function() {
      ...
      await expect(
        intentSource.connect(otherPerson).withdrawRewards(
          routeHash,
          evmIntent.reward
        )
      )
  ...
  describe('Refunding', function() {
    ...
    it('should allow refunding after deadline if intent is not proven', async function() {
      ...
      // Initial balances
      const initialEthBalance = await ethers.provider.getBalance(await creator.getAddress())
      const initialTokenABalance = await tokenA.balanceOf(await creator.getAddress())
      const initialTokenBBalance = await tokenB.balanceOf(await creator.getAddress())
      
      // Execute refund
      await intentSource.connect(otherPerson).refund(routeHash, evmIntent.reward)
      
      // Final balances
      const finalEthBalance = await ethers.provider.getBalance(await creator.getAddress())
      const finalTokenABalance = await tokenA.balanceOf(await creator.getAddress())
      const finalTokenBBalance = await tokenB.balanceOf(await creator.getAddress())
    ...
    it('should prevent refunding if intent is proven', async function() {
      ...
      // Prove the intent
      const [intentHash, routeHash] = await intentSource.getIntentHash(evmIntent)
      await prover.connect(creator).addProvenIntent(intentHash, await claimant.getAddress())
  ...
  describe('Edge cases and validations', function() {
    it('should handle zero token amounts in rewards', async function() {
      ...
      const vaultAddress = await intentSource.intentVaultAddress(zeroAmountIntent)
    ...
    it('should handle intent with only native value rewards', async function() {
      ...
      // Get vault address
      const vaultAddress = await intentSource.intentVaultAddress(nativeOnlyIntent)
      
      // Check vault balance
      expect(await ethers.provider.getBalance(vaultAddress)).to.equal(nativeOnlyIntent.reward.nativeValue)
    ...
    it('should handle malformed addresses gracefully', async function() {
      ...
      expect(await addressConverter.toAddress(zeroBytes32)).to.equal(ZeroAddress)
      
      // Should correctly identify valid vs invalid Ethereum addresses
      expect(await addressConverter.isValidEthereumAddress(zeroBytes32)).to.be.true
      
      // Random bytes32 should usually not be valid Ethereum addresses
      const randomBytes32 = ethers.hexlify(ethers.randomBytes(32))
      const isValidRandom = await addressConverter.isValidEthereumAddress(randomBytes32)
      ...
      if (isValidRandom) {
        ...
        expect(await addressConverter.isValidEthereumAddress(anotherRandomBytes32)).to.be.false
      } else {
