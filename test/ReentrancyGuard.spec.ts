...
describe('Reentrancy Protection', () => {
  ...
  async function deployContractsFixture() {
    ...
    const TestMetaRouterFactory = await ethers.getContractFactory('TestMetaRouter')
    ...
    return { intentSource, inbox, hyperProver, metaProver, legitimateToken, maliciousToken }
  }
  ...
  describe('Token Transfer Reentrancy Protection', () => {
    it('should prevent reentrancy attacks during token transfers', async () => {
      ...
      await maliciousToken.connect(attacker).configureAttack(
      ...
      await maliciousToken.connect(user).approve(intentSource.address, parseEther('10'))
    ...
    it('should handle normal token transfers correctly', async () => {
      ...
      // Approve tokens from user to IntentSource
      await legitimateToken.connect(user).approve(intentSource.address, parseEther('10'))
