...
contract BaseTest is Test {
    ...
    function _setupTestData() internal {
        ...
        TokenAmount[] memory routeTokensMemory = new TokenAmount[](routeTokens.length);
        ...
        TokenAmount[] memory rewardTokensMemory = new TokenAmount[](rewardTokens.length);
    ...
    function _hashIntent(Intent memory _intent) internal pure virtual returns (bytes32) {
    ...
    function _addProof(bytes32 intentHash, uint96 destinationChainId, address recipient) internal {
        vm.prank(creator);
        prover.addProvenIntent(intentHash, destinationChainId, recipient);
    }
    ...
    function _publishAndFundWithValue(Intent memory _intent, bool allowPartial, uint256 value) internal {
        vm.prank(creator);
