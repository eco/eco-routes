 ...
 *   TRON_USDT_B58         (default: TTWQgxc52wHxuuG9sy2D7XFkNcSSzKK7ZB)
 ...
 *     npx ts-node scripts/run-tron-evm-intent_usdt.ts
 */
...
async function main() {
  ...
  const tronUsdtB58Env = process.env.TRON_USDT_B58        || 'TTWQgxc52wHxuuG9sy2D7XFkNcSSzKK7ZB'
  ...
  // Convert USDT base58 address to hex20 for use in ABI encoding
  const tronUsdtHex = '0x' + (tw.address.toHex(tronUsdtB58Env) as string).slice(2)
