...
class TestHelper {
  ...
  async setup() {
    ...
    const AddressConverterTest = await ethers.getContractFactory(
  ...
  describe('AddressConverter', function () {
    it('Should convert address to bytes32 and back', async function () {
      ...
      const recoveredAddress =
        await helper.addressConverter.toAddress(bytes32Value)
    ...
    it('Should handle array conversions correctly', async function () {
      ...
      for (let i = 0; i < testAddresses.length; i++) {
        const bytes32Value = await helper.addressConverter.toBytes32(
          testAddresses[i],
        )
        const recoveredAddress =
          await helper.addressConverter.toAddress(bytes32Value)
        expect(recoveredAddress.toLowerCase()).to.equal(
          testAddresses[i].toLowerCase(),
        )
      }
    })
    ...
    it('Should correctly identify valid Ethereum addresses in bytes32', async function () {
      ...
      const validBytes32 = await helper.addressConverter.toBytes32(
      ...
      const isValid =
        await helper.addressConverter.isValidEthereumAddress(validBytes32)
      const isInvalid =
        await helper.addressConverter.isValidEthereumAddress(invalidBytes32)
